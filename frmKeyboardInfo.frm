...
Private Sub Form_Load()
    ...
    txtLayoutName.Text = LangIdent(LO_WORD(GetKeyboardLayout(0)))
    
    Dim KeyboardLayout As String * 9 '8 bytes + 1 identifier
    If GetKeyboardLayoutName(KeyboardLayout) = False Then Failed "GetKeyboardLayoutName"
    txtLayoutID.Text = Right$(Fix_NullTermStr(KeyboardLayout), 4)
    
    txtSubType.Text = CStr(GetKeyboardType(1))
    ...
    Select Case GetKeyboardType(0)
        ...
        Case Else: txtType.Text = "Unknown / Not Specified"
    End Select
End Sub
