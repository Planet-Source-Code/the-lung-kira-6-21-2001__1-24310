...
Private Sub Form_Load()
    ...
    If WinVersion(-1, 5000000, True) = True Then
        ...
        If GetVersionEx(OSVERSIONINFOEX) = 0 Then Failed "GetVersionEx"
        
        
        With OSVERSIONINFOEX
            txtServicePackVersion.Text = CStr(.wServicePackMajor) & "." & CStr(.wServicePackMinor)
    ...
    Else
        ...
        txtServicePackVersion.Enabled = False
    ...
    If WinVersion(0, -1, True) = True Then
        txtName.Text = GetRegSetting(HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion", "Version")
        txtPlatformID.Text = "WIN32_WINDOWS"
        txtProdID.Text = GetRegSetting(HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion", "ProductId")
        lblRemoteSession.Enabled = False
    Else
        txtName.Text = GetRegSetting(HKEY_LOCAL_MACHINE, "SOFTWARE\Microsoft\Windows NT\CurrentVersion", "ProductName")
        txtPlatformID.Text = "WIN32_NT"
        txtProdID.Text = GetRegSetting(HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows NT\CurrentVersion", "ProductId")
        chkRemoteSession.value = CStr(GetSystemMetrics(SM_REMOTESESSION))
    End If
    ...
    chkDBCS.value = CStr(GetSystemMetrics(SM_DBCSENABLED))
    chkDebug.value = CStr(GetSystemMetrics(SM_DEBUG))
    chkPenExtensions.value = CStr(GetSystemMetrics(SM_PENWINDOWS))
    
    If WinVersion(4010000, -1, False) = True Then
        Dim boolPlus As Boolean
        If SystemParametersInfo(SPI_GETWINDOWSEXTENSION, boolPlus, 0, 0) = 0 Then Failed "SystemParametersInfo"
        chkPlus.value = boolPlus
    Else
        lblPlus.Enabled = False
    End If
    
    chkSecurity.value = CStr(GetSystemMetrics(SM_SECURE))
    txtVersion.Text = CStr(OSVERSIONINFO.dwMajorVersion) & "." & CStr(OSVERSIONINFO.dwMinorVersion) & "." & CStr(OSVERSIONINFO.dwBuildNumber)
End Sub
